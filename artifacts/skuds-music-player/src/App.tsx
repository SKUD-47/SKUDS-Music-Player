...
function TrackMenu({
  ...
  const library = useLibraryContext();
return <div data-track-menu onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} className="fixed right-0 z-40 w-48 rounded-xl border border-border bg-popover p-1.5 shadow-2xl"><button type="button" onClick={() => { library.playSong(song.id); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Play size={14}/> Play now</button><button type="button" onClick={() => { library.addQueue(song.id); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><ListMusic size={14}/> Add to queue</button><button type="button" onClick={() => { onAddPlaylist(); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Plus size={14}/> Add to playlist</button><button type="button" onClick={() => { onInfo(); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Info size={14}/> Song info</button><button type="button" onClick={() => { onEdit(); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Pencil size={14}/> Edit song</button><button type="button" onClick={() => { if (onRemove) { onRemove(); } else { void library.removeSong(song.id); } onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-destructive hover:bg-destructive/10"><Trash2 size={14}/> {removeLabel}</button></div>;
}
...
<IconButton label={`Playlist options for ${song.title}`} onClick={() => setMenuId(menuId === song.id ? null : song.id)}><MoreHorizontal size={16}/></IconButton>{menuId === song.id && <TrackMenu
  song={song}
  onClose={() => setMenuId(null)}
 onAddPlaylist={() => setPlaylistSong(song)}
