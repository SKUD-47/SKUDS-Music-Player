...
const importFiles = useCallback(async (input: FileList | File[]) => {
  ...
  for (const file of files) {
    if (!(ACCEPTED.includes(file.type) || EXTENSIONS.test(file.name))) {
      rejected++;
      continue;
    }
    ...
    try {
      ...
      if (picture?.data?.length) {
        const artworkBytes = picture.data.buffer.slice(
          picture.data.byteOffset,
          picture.data.byteOffset + picture.data.byteLength
        ) as ArrayBuffer;

        embeddedArtwork = new Blob([artworkBytes], {
  ...
  // Create/update the import playlist AFTER all songs have been saved.
  if (addedSongIds.length > 0) {
    ...
    const playlist: StoredPlaylist = existingPlaylist
      ? {
          ...existingPlaylist,
          songIds: [
            ...existingPlaylist.songIds,
            ...addedSongIds.filter(
              (id) => !existingPlaylist.songIds.includes(id)
            ),
          ],
          updatedAt: Date.now(),
        }
    ...
    try {
      ...
      toast(
        'Songs imported, but the playlist could not be created.',
        'error'
      );
    }
  }
  ...
  if (added) {
    toast(
      `${added} ${added === 1 ? 'track' : 'tracks'} added to your library.`
    );
  }
