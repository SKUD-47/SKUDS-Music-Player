  ...
  <div className="mt-6 overflow-hidden rounded-2xl border border-border bg-card/70">
    {songs.map((song, index) => (
      <div
        ...
        className={`list-row flex items-center gap-3 border-b border-border/70 px-3 py-2.5 last:border-0 sm:px-4 cursor-grab select-none ${
      ...
      >
        ...
        <span
          className="shrink-0 cursor-grab text-lg leading-none text-muted-foreground/50"
        ...
        <div className="flex">
          <div className="relative">
            <IconButton
              label={`Playlist options for ${song.title}`}
              onClick={() =>
                setMenuId(menuId === song.id ? null : song.id)
              }
            >
            ...
            {menuId === song.id && (
              <div className="absolute right-0 top-10 z-10 w-40 rounded-xl border border-border bg-popover p-1 shadow-xl">
                <button
                  type="button"
                  className="w-full rounded-lg px-3 py-2 text-left text-xs text-destructive hover:bg-muted"
                  onClick={async () => {
                    await library.updatePlaylist({
                      ...playlist,
...
>
  ...
  <button
    type="button"
    onClick={() => library.playSong(song.id, songs.map((item) => item.id))}
    className="flex min-w-0 flex-1 items-center gap-3 text-left"
  >
    <Artwork song={song} size="sm"/>
    <span className="min-w-0">
  ...
  <div className="flex">
    <div className="relative">
      <IconButton
        label={`Playlist options for ${song.title}`}
        onClick={() => setMenuId(menuId === song.id ? null : song.id)}
      >
        <MoreHorizontal size={16}/>
      </IconButton>
...
</div>)}</div>{songs.map((song, index) => <div
  key={song.id}
  draggable
  onDragStart={() => {
    setDraggedIndex(index);
