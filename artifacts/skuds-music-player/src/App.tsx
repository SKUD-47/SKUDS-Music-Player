...
return <div className="dialog-backdrop fixed inset-0 z-[55] flex items-center justify-center p-4"><div className="relative w-full max-w-lg rounded-3xl border border-border bg-card p-6 shadow-2xl sm:p-10"><IconButton label="Close expanded player" onClick={onClose} className="absolute right-4 top-4"><X size={18}/></IconButton><div className="mx-auto max-w-[330px]"><Artwork song={current} size="hero"/></div><div className="mt-7 text-center"><div className="font-display text-2xl font-semibold">{current.title}</div><div className="mt-1 text-sm text-muted-foreground">{current.artist} · {current.album}</div></div><div className="mt-8 flex items-center gap-2"><span className="font-mono text-[10px] text-muted-foreground">{formatTime(library.progress)}</span><input aria-label="Expanded seek" type="range" className="progress-track min-w-0 flex-1" min="0" max={library.duration || current.duration || 1} value={library.progress} onChange={(event) => library.seek(Number(event.target.value))}/><span className="font-mono text-[10px] text-muted-foreground">{formatTime(library.duration || current.duration)}</span></div><div className="mt-7 flex items-center justify-center gap-5"><IconButton label="Shuffle" onClick={() => library.setShuffle(!library.shuffle)} active={library.shuffle}><Shuffle size={18}/></IconButton><IconButton label="Previous track" onClick={library.previous}><SkipBack size={20}/></IconButton><button type="button" onClick={library.togglePlay} aria-label="Play or pause" className="green-glow flex h-14 w-14 items-center justify-center rounded-full bg-primary text-primary-foreground">{library.isPlaying ? <Pause size={23} fill="currentColor"/> : <Play size={23} fill="currentColor"/>}</button><IconButton label="Next track" onClick={library.next}><SkipForward size={20}/></IconButton><IconButton label="Repeat" onClick={library.cycleRepeat} active={library.repeat !== 'off'}>{library.repeat === 'one' ? <Repeat1 size={18}/> : <Repeat size={18}/>}</IconButton></div><div className="mt-1 flex justify-center"><LyricsFeature song={current} currentTime={library.progress} onSeek={library.seek}/></div><div className="mx-auto mt-3 flex max-w-[180px] items-center gap-2">
  ...
  <input
    ...
    onChange={(event) =>
      library.setVolume(Number(event.target.value))
    }
  />
...
function ArtworkUploadDialog({ title, initialArtwork, onClose, onSave, allowRemove = true }: { title: string; initialArtwork?: Blob; onClose: () => void; onSave: (artwork?: Blob) => Promise<void>; allowRemove?: boolean }) {
  ...
  return <Dialog title={title} onClose={onClose} footer={<><button type="button" onClick={onClose} className="button-ghost rounded-xl px-4 py-2 text-sm">Cancel</button>{allowRemove && initialArtwork && <button type="button" disabled={saving} onClick={async () => { setSaving(true); try { await onSave(undefined); onClose(); } catch (reason) { setError(reason instanceof Error ? reason.message : 'The artwork could not be removed.'); } finally { setSaving(false); } }} className="button-ghost mr-auto rounded-xl px-4 py-2 text-sm text-destructive">Remove</button>}<button type="button" disabled={!source || saving} onClick={() => void save()} className="button-primary rounded-xl px-4 py-2 text-sm font-semibold">{saving ? 'Saving…' : 'Save artwork'}</button></>}>
    <div className="flex flex-col gap-4 sm:flex-row">
      <div className="flex w-full shrink-0 justify-center sm:w-48"><div className="relative h-48 w-48 shrink-0 overflow-hidden rounded-2xl border border-border bg-muted/60">{sourceUrl ? <img src={sourceUrl} alt="Artwork crop preview" className="h-full w-full object-cover" style={{ objectPosition: `${focusX * 100}% ${focusY * 100}%` }}/> : <div className="flex h-full items-center justify-center px-6 text-center text-xs text-muted-foreground">Choose an image to preview the saved square crop.</div>}</div></div>
...
function TrackMenu({
  ...
  const library = useLibraryContext();
return <div data-track-menu onPointerDown={(event) => event.stopPropagation()} onClick={(event) => event.stopPropagation()} className="absolute right-0 top-10 z-[9999] w-48 rounded-xl border border-border bg-popover p-1.5 shadow-2xl pointer-events-auto"
><button type="button" onClick={() => { library.playSong(song.id); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Play size={14}/> Play now</button><button type="button" onClick={() => { library.addQueue(song.id); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><ListMusic size={14}/> Add to queue</button><button type="button" onClick={() => { onAddPlaylist(); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Plus size={14}/> Add to playlist</button><button type="button" onClick={() => { onInfo(); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Info size={14}/> Song info</button><button type="button" onClick={() => { onEdit(); onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs hover:bg-muted"><Pencil size={14}/> Edit song</button><button type="button" onClick={() => { if (onRemove) { onRemove(); } else { void library.removeSong(song.id); } onClose(); }} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-xs text-destructive hover:bg-destructive/10"><Trash2 size={14}/> {removeLabel}</button></div>;
}
